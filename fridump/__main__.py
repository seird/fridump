...
def main():
    ...
    APP_NAME = int(arguments.process) if arguments.process.isdecimal() else arguments.process
    DIRECTORY = ""
    USB = arguments.usb
    ...
    try:
        if USB:
            ...
            session = frida.get_device_manager().add_remote_device(HOST).attach(APP_NAME)
        else:
